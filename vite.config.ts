 ...
 * resolved from a `\0virtual:` importer. This shim rewrites them to absolute
 * paths — scoped strictly to that virtual module, so it never touches normal
 * app imports. (Known workaround, proven in nedb-links; upstream fix queued
 * for portal-core.)
 */
...
export default defineConfig(({ mode }) => {
  ...
  // SALON_API_PORT = Express API server (canonical; falls back to PORT)
  // VITE_PORT      = Vite dev client (what the browser connects to in dev)
  // SALON_API_URL  = full override for the API proxy target (optional)
  ...
  const apiPort = env.SALON_API_PORT || env.PORT || "3201";
  const clientPort = Number(env.VITE_PORT || 3000);
  const apiTarget = env.SALON_API_URL || `http://localhost:${apiPort}`;

  // Paths the DEV SERVER owns (the SPA + Vite internals). Everything else —
  // public pages, sitemaps, /api — belongs to the Express server, exactly
  ...
  function proxyHints(proxy: Parameters<NonNullable<ProxyOptions["configure"]>>[0]): void {
    ...
    proxy.on("error", (err) => {
      ...
      warned = Date.now();
      console.error(
        `\n[salon] API proxy → ${apiTarget} failed: ${err.message}\n` +
